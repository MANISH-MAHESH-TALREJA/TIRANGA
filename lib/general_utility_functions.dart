...
Future<void> mediaShare(String url, String media, String mediaType) async {
  ...
  if (await check()) {
    try {
      ...
      final file = File('${tempDir.path}/TIRANGA.$extension');
...
{
  late Stream<String> progressString = Wallpaper.imageDownloadProgress(url, imageName: "TIRANGA-IMAGE", location: DownloadLocation.temporaryDirectory);
  ...
  }, onDone: () async
  {
    ...
    {
      case 1:
        await Wallpaper.homeScreen(imageName: "TIRANGA-IMAGE", location: DownloadLocation.temporaryDirectory, options: RequestSizeOptions.resizeFit, width: MediaQuery.of(context).size.width, height: MediaQuery.of(context).size.height);
        showToast("$screen WALLPAPER CHANGED SUCCESSFULLY !!!!");
        break;
      case 2:
        await Wallpaper.lockScreen(imageName: "TIRANGA-IMAGE", location: DownloadLocation.temporaryDirectory, options: RequestSizeOptions.resizeFit, width: MediaQuery.of(context).size.width, height: MediaQuery.of(context).size.height);
        showToast("$screen WALLPAPER CHANGED SUCCESSFULLY !!!!");
        break;
      case 3:
        await Wallpaper.bothScreen(imageName: "TIRANGA-IMAGE", location: DownloadLocation.temporaryDirectory, options: RequestSizeOptions.resizeFit, width: MediaQuery.of(context).size.width, height: MediaQuery.of(context).size.height);
        showToast("$screen WALLPAPER CHANGED SUCCESSFULLY !!!!");
        break;
      default:
...
void shareMe()
{
  SharePlus.instance.share(
      ShareParams(title: 'SHARE TIRANGA APP', text: "SHARE TIRANGA - VIRTUAL INDEPENDENCE DAY APP MADE BY RAAM DEVELOPERS. YOU CAN DOWNLOAD THE APP BY CLICKING ON THE LINK BELOW FROM GOOGLE PLAY STORE. \n\n${Constants.AppPlayStoreLink}")
  );
}
