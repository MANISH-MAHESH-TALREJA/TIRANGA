...
class HtmlOldParser extends StatelessWidget {
  ...
  List<Widget> parse(String data) {
    ...
    for(int i = 0; i < document.body!.nodes.length; i++)
    {
      widgetList.add(_parseNode(document.body!.nodes[i])); // MAJOR CHANGE
    }
    return widgetList;
  }
    ...
    if (node is dom.Element) {
      ...
      switch (node.localName) {
        ...
        case "li":
          ...
          switch (type) {
            case "ul":
              mark = Container(padding: markPadding, child: const Text('•'));
              break;
            case "ol":
            ...
            painter = TextPainter(
                text: TextSpan(
                  ...
                  style: parentStyle.merge(TextStyle(
                      fontSize:
                          parentStyle.fontSize! * offsetTagsFontSizeFactor)),
                ),
                textDirection: TextDirection.ltr);
            painter.layout();
            //print(painter.size);
      ...
      if (finalText.endsWith(" ")) {
        return Container(
            padding: const EdgeInsets.only(right: 2.0), child: Text(finalText));
      } else {
